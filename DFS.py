...
# a node class for building the state tree
class Node:
    def __init__(self, state, parent = None, edge = None):
        # the state that was discovered at this position
        self.state = state

        # the edge from which this state was discovered
        self.edge = edge
...
def depth_first_search(origin):
    ...
    while frontier:
        ...
        for edge in reversed(branch.state.edges):
            ...
            frontier.append(Node(edge.node_to, branch, edge))
