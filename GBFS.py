...
# performs Greedy Best-First Search from a given origin vertex using a given heuristic
def greedy_best_first_search(origin, heuristic):
    root = Node(origin)

    # create the initial frontier, containing only the origin node
    frontier = PriorityQueue()

    # set the root node value using only the heuristic
    root.value = heuristic.judge(root)
    ...
    while frontier.qsize() > 0:
        # consider the next node in the frontier with the lowest heuristic value
        branch = frontier.get()
        ...
        for edge in branch.state.edges:
            ...
            # for GBFS, only the heuristic value is used to evaluate the node
            new_node.value = heuristic.judge(new_node)
